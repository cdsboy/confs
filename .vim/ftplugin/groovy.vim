...
let s:GROOVY_PATH = s:GROOVY_HOME . "/usr/bin/groovy"
let s:GROOVYC_PATH = s:GROOVY_HOME . "/usr/bin/groovyc"
...
if !hasmapto("<SID>ShowHelp()", "n")
	nmap <buffer><unique><silent> <F7> :call <SID>ShowHelp()<CR>
endif
...
if !hasmapto("<SID>CompilePrompt()")
	nmap <buffer><unique><silent> <F8> :call <SID>CompilePrompt()<CR>
endif
...
au VimEnter *.groovy echo "F7 for Groovy plugin help"
